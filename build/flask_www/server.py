...
def index():
    ...
    monthly=query_db("select strftime('%Y', data) as year, strftime('%m', data) as month, min(ilosc) as minimum, max(ilosc) as maximum, round(avg(ilosc), 1) as average, min(iloscz) as minimumo, max(iloscz) as maximumo, round(avg(iloscz), 1) as averageo from cytrynki where ilosc > 0 group by strftime('%Y', 'data'), strftime('%m',data) order by year desc, month desc")
